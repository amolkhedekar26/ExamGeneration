<template>
  <div>
    <h3>Dynamic accordion using VueJS</h3>
    <p>
      Here's a very simple and transparent implementation of dynamic accordion
      using VueJS 2. No third-party packages are needed for this project.
    </p>
    ...
    <div class="card" v-for="item in items" :key="item.id">
      <div class="card-header" @click.prevent="toggleExpand(item)">
        ...
        <div class="icon">
          ...
          <span class="fs-4" v-if="!item.isExpand">+</span>
          <span class="fs-4" v-if="item.isExpand">-</span>
          <!-- <i class="fas fa-arrow-up" v-if="item.isExpand"></i> -->
          <i class="bx bx-chevron-up" v-if="!item.isExpand"></i>
        </div>
      </div>
